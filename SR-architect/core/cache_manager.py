"""
Cache Manager for SR-Architect pipeline.

Provides SQLite-based caching for:
- Document parsing results (skip re-parsing)
- Field extraction results (skip re-extraction)
- Embedding vectors (skip re-embedding)

Per spec.md, uses simple invalidation policy:
- Document cache: Invalidate on parser version change
- Field cache: Invalidate on schema version change
...
DEFAULT_CACHE_PATH = Path(__file__).parent.parent / ".cache" / "extraction_cache.db"
...
class CacheManager:
    ...
    def set_field(
        ...
        Args:
            ...
            tier_used: Extraction tier (0=regex, 1=local, 2=cloud, etc.)
